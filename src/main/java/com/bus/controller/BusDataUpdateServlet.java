...
public class BusDataUpdateServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("busdataupdate".equals(action)) { // accountCenter.jsp裡的請求
			...
			BusDAO_interface busDAOInterface = new BusDAO();// 查詢帳號密碼的方法寫在DAO(GET_TWO_STMT)
			BusVO busbean = busDAOInterface.selectByEmailAndPassword(email, old_BusPwd);
			// 若有busbean表示密碼有輸入正確
			if (busbean != null) {
				// 必填輸入欄(姓名、電話、地址)的資料有符合格式的話，client端所填之內容可進入DB更新
				if (!"null".equals(newname) && !"null".equals(newphone) && !"null".equals(newaddress)) {
...
//					
					...
					req.getRequestDispatcher("../nest-backend/busAccountCenter.jsp").forward(req, res);
				...
				req.setAttribute("errBusPWMsg", "不好意思!您密碼輸入錯誤或無填寫密碼!請確實填寫正確密碼後，方可儲存欲修改資料!!");
				req.getRequestDispatcher("../nest-backend/busAccountCenter.jsp").forward(req, res);
			}
