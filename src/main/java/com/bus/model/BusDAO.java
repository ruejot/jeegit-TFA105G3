...
public class BusDAO implements BusDAO_interface {
	...
	// 新增
	private static final String INSERT_STMT = "INSERT INTO BUS (NAME, PHONE, ADDRESS, TAX_ID, DATE, EMAIL, PASSWORD, INTRO, PHOTO, FB, IG, WEBSITE, PAYMENT_PROVIDE)"
			+ " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
	// 修改
	private static final String UPDATE = "UPDATE BUS SET NAME=?, PHONE=?, ADDRESS=?, TAX_ID=?, DATE=?, EMAIL=?, PASSWORD=?, INTRO=?, PHOTO=?, FB=?, IG=?, WEBSITE=?, PAYMENT_PROVIDE=?"
			+ " WHERE BUS_ID=?";

	// 忘記密碼後的變更密碼
	private static final String UPDATEPASSWORD = "UPDATE BUS SET PASSWORD=? WHERE BUS_ID=?";
	// 刪除
	private static final String DELETE = "DELETE FROM BUS WHERE BUS_ID=?";
	// 查詢by Bus_ID
	private static final String GET_ONE_STMT = "SELECT BUS_ID, NAME, PHONE, ADDRESS, TAX_ID, DATE, EMAIL, PASSWORD, INTRO, PHOTO, FB, IG, WEBSITE, PAYMENT_PROVIDE FROM BUS WHERE BUS_ID = ?";
	// 查詢Bus_ID+EMAIL BY EMAIL
	private static final String GET_ID_EMAIL = "SELECT BUS_ID, EMAIL FROM BUS WHERE EMAIL =?";
	// 查詢by EMAIL
	private static final String GET_EMAIL_STMT = "SELECT BUS_ID, NAME, PHONE, ADDRESS, TAX_ID, DATE, EMAIL, PASSWORD, INTRO, PHOTO, FB, IG, WEBSITE, PAYMENT_PROVIDE FROM BUS WHERE EMAIL =?";
	// 查詢by EMAIL and PASSWORD
	private static final String GET_TWO_STMT = "SELECT BUS_ID, NAME, PHONE, ADDRESS, TAX_ID, DATE, EMAIL, PASSWORD, INTRO, PHOTO, FB, IG, WEBSITE, PAYMENT_PROVIDE FROM BUS WHERE EMAIL =? and PASSWORD = ?";
	// 查詢全部
	private static final String GET_ALL_STMT = "SELECT BUS_ID, NAME, PHONE, ADDRESS, TAX_ID, DATE, EMAIL, PASSWORD, INTRO, PHOTO, FB, IG, WEBSITE, PAYMENT_PROVIDE FROM BUS ORDER BY BUS_ID";
	...
	// 忘記密碼後的變更密碼
	@Override
	public void updateBusPW(BusVO busBean) {
		Connection con = null;
		PreparedStatement pstmt = null;
		...
		try {
			...
			pstmt.setString(1, busBean.getPassword());
			pstmt.setInt(2, busBean.getBusid());
